...
def bump_sw(version):
    """Naikkan VERSION dalam sw.js ke versi data baharu supaya PWA auto-update."""
    import re
    ...
    new = re.sub(r"const VERSION = '[^']*';",
                 "const VERSION = 'hosza-audit-%s';" % version, txt, count=1)
    if new != txt:
        with open(SW, "w", encoding="utf-8") as f:
            f.write(new)
        print("sw.js VERSION : hosza-audit-%s" % version)
