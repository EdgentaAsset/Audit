...
Selepas jana semula, **naikkan nombor versi** dalam `sw.js` (lihat Bahagian 5) supaya
pengguna dapat data terbaharu secara automatik.
...
- **Auto-update:** setiap deploy/kemaskini data, naikkan versi dalam `sw.js`:
  ```js
  const VERSION = 'hosza-audit-v2';   // v1 → v2 → v3 …
  ```
